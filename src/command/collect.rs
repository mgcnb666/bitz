...
impl Miner {
    ...
    async fn find_hash_par(
        ...
        user_min_score: Option<u32>,
    ) -> Solution {
        // Dispatch job to each thread
        let progress_bar = Arc::new(spinner::new_progress_bar());
        let global_best_difficulty = Arc::new(RwLock::new(0u32));
        let continue_calculating = Arc::new(RwLock::new(true));
        ...
        let handles: Vec<_> = core_ids
            .map(|i| {
                ...
                let continue_calculating = Arc::clone(&continue_calculating);
                std::thread::spawn({
                    let progress_bar = progress_bar.clone();
                    let nonce = nonce_indices[i.id];
                    let mut memory = equix::SolverMemory::new();
                    let pool_channel = pool_channel.clone();
                    let user_min_score = user_min_score;
                    move || {
                        // Pin to core
                        ...
                        loop {
                            // Check if we should continue calculating
                            if !*continue_calculating.read().unwrap() {
                                break;
                            }
                            ...
                            if nonce % 100 == 0 {
                                ...
                                if timer.elapsed().as_secs().ge(&cutoff_time) {
                                    if i.id == 0 {
                                        // 如果用户设置了最低score，并且当前score低于设置值，则显示提示信息
                                        if let Some(user_min) = user_min_score {
                                            if global_best_difficulty < user_min {
                                                ...
                                                // 设置计算结束标志
                                                *continue_calculating.write().unwrap() = false;
                                                break;
                                            }
                                        } else {
                                            progress_bar.set_message(format!(
                                    ...
                                    if let Some(user_min) = user_min_score {
                                        ...
                                        // 未达到用户设置的分数，继续计算
                                    } else if global_best_difficulty.ge(&min_difficulty) {
                                        // 没有用户设置的分数，使用系统默认的min_difficulty，结束计算
                                        *continue_calculating.write().unwrap() = false;
                                        break;
                                    }
                                } else if i.id == 0 {
